...
//TAS: Decide which child to swap with parent during reheapify
{
  ...
  else if ((tree[left].getPriority() >  
            tree[right].getPriority()))
...
//POS: heap is a heap
  ...
  while ((next < insertPoint) &&
         (tree[next].getPriority() > tree[node].getPriority())){
      //swap node and next
      swap(node, next);
  ...
  //TAS:
  {
    HeapNode oldRoot = tree[0];
    boolean done = false;
    int parent = (insertPoint-1)/2;
    int child = insertPoint;
    if(insertPoint == 1)  // if there is only one node to be removed from the heap.
    {
      tree[0] = null; // root
